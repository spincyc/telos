...
| 6 | Windows join and login | Join the synthetic domain; prove secure channel, DNS SRV, time, named user login, named administrator elevation, `local-rescue`, reboot, cached offline login, update policy, and recovery path. | in progress 2026-08-11, 8/24 checks passing (attempt `20260811T185946Z-945648092667`, `acceptance-progress.json`): controller-ready, windows-joined, windows-standard-online, windows-daily-admin, domain-admin-separate, windows-rebooted-joined, windows-cached-policy, and controller-offline all pass. The whole credential-proof mechanism was rebuilt this session: the post-reboot domain logon (departure-band, realm-on-the-wire, receipt parse), the first-logon Start-menu dismissal, a token-based LogonUser proof replacing an unusable process-spawn (Kerberos package via LsaGetLogonSessionData, UPN principal match, raw-token-groups local-admin detection), and full fault-phase coordinate instrumentation. Next wall: check 9 windows-cached-login times out against the SIGSTOP-frozen DC (offline-login path) — under diagnosis. |
