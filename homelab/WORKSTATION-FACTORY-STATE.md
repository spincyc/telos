...
| 6 | Windows join and login | Join the synthetic domain; prove secure channel, DNS SRV, time, named user login, named administrator elevation, `local-rescue`, reboot, cached offline login, update policy, and recovery path. | ALL 24 CHECKS PASS 2026-08-12 (attempt `20260812T043214Z-28ff545de0ce`, acceptance-progress.json passed 24/24): join, standard/daily-admin/domain-admin-separate logins, reboot-rejoin, cached policy, controller-offline, cached standard+admin offline login, uncached-denied, local-rescue, controller/secure-channel restored, update policy, gateway/update-source/ad-dns/combined-dependency outages, services-restored, diagnostics-sanitized, and the aggregate. That run also executed the recovery step, which by design DESTROYS the one-use credential-bearing recovery publication -- so the retained gate-5 bundle's publication.iso is now consumed. The final evidence PUBLISH surfaced three over-strict aggregate `_expect` assertions in sequence (each new confirm run reveals the next, since publish stops at the first): (1) `controller-ready.synthetic_directory` unprovable pre-join → relocated to windows-standard-online (`f8b0c24`); (2) reference disk-hash pinned to the original install → made references version-portable (`4e28609`); (3) `windows-secure-channel-restored.secure_channel` came back false because read-only `Test-ComputerSecureChannel` hits Netlogon's post-outage negative cache → the domain-state probe now re-verifies with a bounded retry and `-Repair` re-establish (this session, `Invoke-TelosIdentityProbe.ps1`); the same fix covers `windows-services-restored.secure_channel`. Static audit of the remaining publish assertions (update-policy, fault reachabilities, diagnostics-sanitized, meta) shows they are install-deterministic or host-side and were all passed as checks. A fresh gate-5 install (`run-20260812T183626Z-8b9846e74dab`, in progress) is minting a matching bundle for the confirming clean publish, which emits the windows-evidence gate 8 consumes. The entire credential-proof mechanism was rebuilt this session (token LogonUser, Kerberos package via LSA, raw-token-groups membership, live-tolerant secret scanner, root-cause instrumentation). |
