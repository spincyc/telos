...
Document version: `20260812.002`

Status: active implementation

Last evidence/workstream review: 2026-08-11T06:35:00-05:00

Repository baseline reviewed: `a179368`
...
| 4 | PXE authority boundary | Simulated gateway remains sole DHCP authority; controller supplies only the approved boot and identity services; packet evidence proves no rogue offer, forwarding, or external connection. | PASS 2026-08-12 on the arch factory: the read-only `homelab-pxe-authority-audit` renders VERDICT PASS on the real gate-7 run `arch-installs/run-20260811T170109Z-7ceb936e2710` switch.jsonl -- gateway is the sole DHCP authority, the controller emits no DHCP and no identity announcement, every controller flow is within the real Samba-AD/PXE served surface, and nothing egresses an unenumerated endpoint. Wired into factory-verify as a distinct gate (`5fb7829`); a controller DHCP offer, controller identity announcement, or backdoor listen/egress still fail. |
...
| 8 | Arch join and login | Prove SSSD identity, UID/GID stability, Kerberos time, named user and administrator behavior, reboot, cached offline login, automatic-update gate, rollback, and local rescue. | live boundary wired `e58cef8` (controller bring-up, identity fabric, disk-boot workstation, bounded duration) with bundle producer `arch_identity_prepare`; synthetic principals now carry POSIX attrs (`cf13ebd`, staged per run, no image rebuild); menu-drive + operator serial login lane in progress. **First live gate-8 run 2026-08-13** (bundle `arch-identity/run-20260813T200144Z-108311da65fd`, prepared from the passing gate-6 evidence + gate-7 disk `run-20260811T141601Z`): failed fast (~6min) with "systemd-boot menu never rendered on the workstation serial console". DIAGNOSIS: the DISK-side gate-8 provisioning is present in `arch_second.py` and the 141601Z install.sh (probe helper `/usr/local/sbin/homelab-arch-identity-probe`, `serial-getty@ttyS0`, kernel cmdline `console=tty0 console=ttyS0,115200`, sudoers), but the systemd-boot MENU renders to the OVMF/UEFI console, which the arch-identity boundary (`arch_identity_run.py`, boots bundle `OVMF_VARS.fd` + NVMe) does not route to ttyS0 — whereas gate-10 dualboot DID render the menu on serial (it pairs the VARS with the gate-7 installed OVMF and the EFI stub prints on the OVMF console/serial, `dualboot_acceptance.py`). NEXT: make the arch-identity boundary source/configure OVMF for serial-console redirection like the dualboot boundary (compare OVMF pairing + `-serial`/console vars), then re-run; a fresh gate-7 Arch install with current provisioning may also be needed if 141601Z predates a serial-console fix. Gate-8 is otherwise wired end-to-end (prepare accepts gate-6 evidence; run + judge execute). |
| 9 | Optional storage failure | Prove per-user SMB authorization when present and successful login with no delay or hard failure when the NAS is absent. Record UID/GID and timestamp measurements before reconsidering NFS. | controller-side COMPLETE and test-locked 2026-08-12 (`11c2c5f`): the converged controller exports the per-user `[homes]` SMB share, maps rfc2307 identities so an owner reads their own `/srv/unas/<user>` and a foreigner is denied, and publishes `unas` to its own address so storage is reachable by default; the gate-8 runner toggles it unreachable in DNS for the absent-login proof. cifs-utils is in the workstation closure. Live proof of the three checks awaits a live gate-8 run. |
| 10 | Dual-boot acceptance | From cold boot, select and log into both systems; verify Windows-default five-second policy, disk measurements, EFI recovery choices, and no cross-OS partition damage. | **PASS 2026-08-11**: bundle `dualboot-acceptance/run-20260811T170510Z-a619bcb1f028` records `observed`/`dualboot-accepted`, all eight checks green. From cold boot the firmware started `Linux Boot Manager` (systemd-boot), the five-second menu rendered Windows-default (measured ~5s), Windows booted and shut down cleanly (retained frames confirm), boot 2 rendered the menu again (Linux-first NVRAM held — Windows adopted the blob-bearing entry and did not self-promote, `c15dff7`/`83be6bf`), the drive paused the countdown and arrow-navigated to Arch then pressed Enter (`6d8823b`), Arch handed off on ttyS0 to its getty login surface, the GPT was byte-unchanged, and both EFI boot managers plus the recovery entry were present. Live-authored Windows login is deferred to gate 6's identity stream. |
| 11 | Lifecycle recovery | Exercise controller restart/loss, PXE release rollback, failed install, broken boot, directory/DNS loss, update failure, workstation remint, and controller reconstruction from public inputs plus a synthetic private overlay. | runner + judge implemented `f39f3a1` (`make homelab-factory-recover`); three scenarios PROVEN live in the loopback lab 2026-08-12 (pxe-release-rollback, update-failure-rollback per ADR 0075, workstation-remint all pass); the five needing a live guest boot (controller-restart, failed-install-recovery, broken-boot-repair, directory-dns-loss, controller-reconstruction) record their observable contract and defer the boot proof, verdict `partial`. |
...
- CURRENT 2026-08-11 (supersedes the two long gate-6/gate-7 investigation
  ...
  zombie at teardown (`a179368`). Still open before the next live gate-7 run:
  host-side join-media wiring (build/attach/destroy the one-use TELOS_JOIN
  credential ISO around the new in-guest join step) — in progress. Gates 8 and
  10 gained runners (`e58cef8`, `3d45584`); both await their first live
  shakedown behind gate 7. No successful local lifecycle permits any implicit
