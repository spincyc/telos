...
\telosmeta{Homelab}{Workstation Owner Guide}
  {Normal use, automatic updates, recovery, and useful evidence}{20260727.002}
...
\section{When the home Controller is unreachable}
...
\begin{enumerate}
\item Connect to a known non-home network and open an ordinary HTTPS site.
\item Sign in with an account that has previously used this laptop.
\item Open one local file without opening an optional network share.
\item Check the operating system's update status.
\item Treat only the home storage or directory operation as unavailable.
\end{enumerate}

\ownercheck{Test login, one local file, public Internet access, and update
status without the home Controller.}{The desktop opens promptly, local work
remains present, Internet access works, and an unavailable home service does
not produce repeated credential prompts.}

\begin{gate}{Stop before changing identity}
Stop if offline login fails, local files disappear, the laptop repeatedly
...
that optional storage reconnects and updates remain current. A Controller
...
\texttt{20260727.002}, all pages render, and the recovery ladder needs no link.}
