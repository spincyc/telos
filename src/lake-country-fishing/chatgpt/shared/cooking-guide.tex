\input{common/preamble.tex}
\input{lake-country-fishing/chatgpt/shared/fish-art/\FishArtSlug.tex}
\newcommand{\FishBonePlate}[3]{%
\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
\path[use as bounding box] (-.2,-1.55) rectangle (7.2,1.75);
\FishOutline\FishDetails
\draw[very thick] (1.15,0)--(6.25,0);
\foreach \x in {1.55,1.9,...,4.7}{\draw[line width=.45pt] (\x,0)--(\x+.26,-.62);}
\draw[very thick,dashed] (1.4,.18)--(#1,.18);
\draw[line width=.4pt] (3.1,.2)--(3.1,1.22);
\node[font=\scriptsize,anchor=south,fill=white] at (3.1,1.25) {#2};
\node[font=\scriptsize,align=center,fill=white] at (3.25,-1.28) {rib sweep: shallow blade\\from backbone toward belly};
\node[font=\scriptsize,align=center,fill=white] at (5.75,1.32) {#3};
\end{tikzpicture}
\end{center}}
\newcommand{\YBonePlate}[2]{%
\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
\path[use as bounding box] (-.2,-1.9) rectangle (7.2,1.85);
\FishOutline\FishDetails
\draw[very thick] (1.2,0)--(6.25,0);
\foreach \x in {1.75,2.12,...,4.34}{
  \draw (\x,.04)--(\x+.18,.42);
  \draw (\x+.18,.42)--(\x+.02,.68);
  \draw (\x+.18,.42)--(\x+.38,.66);
}
\draw[very thick,densely dashed] (1.52,.78)--(4.85,.78);
\draw[very thick,densely dashed] (1.52,.12)--(4.85,.12);
\draw[very thick,densely dashed] (4.85,-.72)--(4.85,.72);
\node[font=\scriptsize,anchor=south,fill=white] at (3.0,1.38) {upper cut above fork tips};
\node[font=\scriptsize,align=center,fill=white] at (5.75,1.35) {#1};
\node[font=\scriptsize,align=center,fill=white] at (3.5,-1.68) {#2};
\end{tikzpicture}
\end{center}}
\providecommand{\CleaningPlate}{%
\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round]
\FishOutline\FishDetails
\draw[densely dashed] (1.35,-.72)--(1.35,.72);
\draw[very thick,-{Latex}] (5.8,-.62)--(1.25,-.62);
\node[font=\scriptsize,fill=white] at (3.6,-.62) {shallow belly opening: vent toward throat};
\node[font=\scriptsize,align=center,fill=white] at (1.35,1.25) {collar cut\\behind gill plate};
\node[font=\scriptsize,align=center,fill=white] at (5.7,1.25) {\SkinNote};
\end{tikzpicture}
\end{center}}
\providecommand{\FilletPlate}{}
\providecommand{\CookingPlate}{}
\providecommand{\DispatchPlate}{}
\telosmeta{Lake Country Fishing · ChatGPT}{Cooking \FishName}{From legal fish to boneless meal}{2026-07-26}
\begin{document}
\telosmasthead[\LatinName\ · kitchen guide]

\begin{teloshazard}{Start with a lawful, cold fish}
Confirm the live Wisconsin DNR rule and consumption advisory for the exact
water, species, length, and date. Kill a retained fish promptly; do not let it
suffocate slowly. Bleed only where lawful and practical, then place it in a
draining ice slurry or on ice. Keep raw fish at or below \qty{40}{^\circ F};
wash hands, knife, and board after contact with viscera.
\end{teloshazard}

\section{Keep or release?}
\begin{tabularx}{\linewidth}{@{}p{.21\linewidth}L@{}}
\toprule
Legal floor & \Legal\\
Preferred table fish & \EatingSize\\
Why & \SizeReason\\
\bottomrule
\end{tabularx}
\textbf{Legal is not the same as desirable.} A fish may be lawful yet a poor
choice because it is a large breeder, exceeds current meal advice, or cannot be
chilled and used promptly.

\section{Dispatch and field care}
\begin{center}
\begin{tikzpicture}[node distance=.5cm, every node/.style={font=\small,align=center}]
\node[draw,minimum width=2.25cm,minimum height=.7cm] (a) {stun firmly\\above brain};
\node[draw,minimum width=2.25cm,minimum height=.7cm,right=of a] (b) {destroy brain\\confirm no reflex};
\node[draw,minimum width=2.25cm,minimum height=.7cm,right=of b] (c) {bleed if chosen\\clean cut at gills};
\node[draw,minimum width=2.25cm,minimum height=.7cm,right=of c] (d) {bag and ice\\drain meltwater};
\draw[-{Latex}] (a)--(b);\draw[-{Latex}] (b)--(c);\draw[-{Latex}] (c)--(d);
\end{tikzpicture}
\end{center}
\DispatchPlate
\begin{enumerate}
\item Use a decisive percussive blow to the skull immediately above and just
behind the eyes. Follow with a spike or narrow knife into the brain cavity.
\item Confirm loss of rhythmic gill movement and eye reflex. If either remains,
repeat immediately. Cut the gill arches only after insensibility.
\item Keep the fish shaded, clean, and cold. Do not soak flesh in warm lake
water. At home, work in small batches and return finished portions to ice.
\end{enumerate}

\newpage
\section{Clean without contaminating the flesh}
\CleaningPlate
\begin{teloscard}{Pause at the plate · what must be true?}
\begin{tabularx}{\linewidth}{@{}p{.28\linewidth}L@{}}
\textbf{Before the opening} & Fish is still at or below
\qty{40}{^\circ F}; clean and waste trays are separate; the knife tip cannot
reach the organs at the planned depth.\\
\textbf{After the opening} & Viscera came out intact; no green gall stain,
fecal material, or abnormal odor is present; kidney tissue is gone.\\
\textbf{If not} & Stop. Isolate the affected side, wash hands and tools, and
trim a generous contaminated margin. If condition or time warm is uncertain,
discard rather than trying to cook safety back into the fish.\\
\end{tabularx}
\end{teloscard}
\begin{enumerate}
\item \textbf{Set up.} Flexible sharp fillet knife, cut-resistant glove on the
non-knife hand, scaler or spoon if needed, pliers, clean towels, two trays.
\item \textbf{Scale or skin.} \ScaleSkin
\item \textbf{Open shallowly.} From vent toward the throat, lift the belly wall
away from organs and use the tip only. Remove viscera intact. Avoid puncturing
the gall bladder; trim and rinse any contaminated tissue.
\item \textbf{Kidney line.} Scrape the dark tissue along the backbone and rinse
briefly with clean potable water. Pat dry. Never use soap on food.
\end{enumerate}

\begin{telospanel}{Parasites and raw fish}
Freezing and pickling are not substitutes for a validated parasite-control
process. Cook wild freshwater fish to an internal temperature of
\qty{145}{^\circ F}, or until opaque and separating easily, unless following a
qualified food-safety process. Discard abnormal-smelling or badly diseased
flesh; when uncertain, do not eat it.
\end{telospanel}

\Needspace{.48\textheight}
\section{Species-specific first cuts}
\FilletMethod
\FilletPlate
\begin{telospanel}{Cut checkpoint}
Before turning the blade, ask: \emph{Can I feel the backbone under the edge,
and is the edge pointing away from my off hand?} After lifting the fillet,
compare both sides. A ragged frame with meat still attached means the blade
angle was too steep; recover that meat with short strokes instead of sawing
blindly. A cut gut means stop, clean the station, and reassess the edible
margin before continuing.
\end{telospanel}

\ifdefempty{\FilletPlate}{}{\newpage}
\section{Bone map and deboning}
\BoneDiagram

\begin{enumerate}
\item Keep the skin side down so the blade can ride on, not through, the bones.
\item Remove the rib cage with a shallow sweep, sacrificing as little belly
meat as possible. Run fingertips both directions along every cut surface.
\item \Debone
\item Portion only after a final fingertip inspection under a bright light.
Bones become harder to locate once batter, crumbs, or sauce is added.
\end{enumerate}

\newpage
\begin{teloscard}{Diagnose before coating}
\begin{tabularx}{\linewidth}{@{}p{.25\linewidth}LL@{}}
\toprule
\textbf{Finding} & \textbf{Likely cause} & \textbf{Correction}\\
\midrule
Bone tips remain & Only one fingertip direction was checked, or the V/channel
was too narrow. & Brush both directions under bright light; recut only the
marked line.\\
Useful meat stays on ribs & Blade was held too upright. & Lay it nearly flat
and make a second shallow sweep; never cut toward the supporting hand.\\
Fillet tears while skinning & Flesh or board is warm/wet, or the blade rose
through the meat. & Rechill, dry the grip end, flatten the blade, and restart
from a sound edge.\\
\bottomrule
\end{tabularx}
\end{teloscard}

\begin{teloscard}{Trim map}
\TrimNotes
\end{teloscard}

\section{Prepare before heat}
\begin{itemize}
\item Pat portions dry. Equal thickness matters more than equal weight.
\item \Prep
\item Refrigerate while the pan, oven, or grill heats. Marinate under
refrigeration; discard marinade that contacted raw fish unless boiled.
\end{itemize}

\begin{center}
\begin{tabularx}{.94\linewidth}{@{}p{.25\linewidth}*{3}{>{\centering\arraybackslash}X}@{}}
\toprule
\textbf{Record before heat} & Thickest piece & Thinnest piece & Batch start\\
\midrule
Thickness / time & \rule{1.4cm}{.4pt} in & \rule{1.4cm}{.4pt} in &
\rule{1.7cm}{.4pt}\\
\bottomrule
\end{tabularx}
\end{center}

\begin{teloscard}{Plan an even batch}
Arrange the portions before heat and mark the thickest center in the sketch
frame below. Which piece should be checked first? Which thin piece may need to
leave the heat early? State the observation—not crust color or elapsed time—
that will trigger each decision.

\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm]
\draw[thin] (0,0) rectangle (10.6,2.35);
\foreach \x in {2.65,5.3,7.95}\draw[thin,dashed] (\x,0)--(\x,2.35);
\node[font=\scriptsize,anchor=north west] at (.12,2.2)
  {Sketch portions in pan / rack order; label thickness and probe entry};
\end{tikzpicture}
\end{center}

\begin{tabularx}{\linewidth}{@{}p{.22\linewidth}p{.17\linewidth}p{.18\linewidth}L@{}}
\toprule
\textbf{Piece / position}&\textbf{Thickness}&\textbf{First check}&
\textbf{Predicted action if below or above target}\\
\midrule
&&&\\[.55em]
&&&\\[.55em]
&&&\\
\bottomrule
\end{tabularx}
\end{teloscard}

\Needspace{.48\textheight}
\section{Two dependable methods}
\CookingPlate
\begin{teloscard}{\CookOne}
\CookOneText
\end{teloscard}
\begin{teloscard}{\CookTwo}
\CookTwoText
\end{teloscard}

\section{Measure, correct, prove}
\begin{enumerate}
\item Insert a clean instant-read probe sideways into the geometric center of
the thickest portion, away from pan, rack, and bone. Record the lowest stable
reading: \rule{1.4cm}{.4pt}\,°F at \rule{1.6cm}{.4pt}.
\item If it is below \qty{145}{^\circ F}, return only underdone pieces to heat
in short increments. Clean the probe, then check a new center. If thin pieces
are done first, remove them rather than drying the whole batch.
\item Split the measured thickest piece. The center must be opaque and separate
easily, with no translucent core. Run one final fingertip pass over the first
serving portion before sauce or garnish hides the surface.
\end{enumerate}

\begin{telospanel}{Final proof}
Serve only when four answers are yes: \textbf{lawful fish? cold chain known?
every portion bone-checked? measured center at least \qty{145}{^\circ F}?}
Move leftovers to shallow containers and refrigerate within two hours. Write
the successful thickness, method, time, and center temperature on this guide;
that record is the starting point for the next batch, not a replacement for
measuring it.
\end{telospanel}

\section{Current safe-eating advice · checked 2026-07-26}
\Advisory

\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm]
\node[draw,minimum width=2.2cm,minimum height=.75cm] at (0,0) {1 serving};
\node[anchor=west,font=\small] at (1.35,.22) {adult: about the palm of the hand};
\node[anchor=west,font=\small] at (1.35,-.22) {child: about the child's palm};
\draw[|-|] (-1.1,-.55)--(1.1,-.55);
\node[font=\scriptsize] at (0,-.9) {count all fish meals from all sources};
\end{tikzpicture}
\end{center}

\begin{teloshazard}{The query beats the printed sheet}
Wisconsin has a statewide inland baseline and water-specific exceptions.
Before eating, use DNR's Fish Consumption Advisory Query for Pine Lake
(WBIC 779200) or North Lake (WBIC 850800), and match species and length. People
who are pregnant, may become pregnant, are breastfeeding, and children need
the more protective category. When advice changes, follow the current query.
\end{teloshazard}

{\footnotesize\textbf{Sources.} Wisconsin DNR Eating Your Catch and 2026--27
water-specific regulations; Wisconsin DHS Eat Fish Safely; USDA/FoodSafety.gov
Safe Minimum Internal Temperature Chart; University of Minnesota Extension,
Preserving Food at Home: Meat, Fish and Poultry. Cleaning sequence and
monochrome diagrams are original editorial synthesis, checked 2026-07-26.}
\end{document}
